...
function main(csv_file::String="")
    ...
    # 输出图像路径
    output_dir = joinpath(script_dir, "../../results/figures/Gas_Liquid")
...
elseif isinteractive() && length(ARGS) == 0
    ...
    main("../../results/output/Gas_Liquid/forwarddiff_optimization_params_scan.csv")
    
else
    println("脚本已加载。请手动调用 main() 或 main(\"csv文件路径\") 来绘制图形。")
    println("示例:")
    println("  main()  # 使用默认路径")
    println("  main(\"../../results/output/Gas_Liquid/your_file.csv\")  # 使用指定路径")
end
